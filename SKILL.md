...
**提示用户可用功能 (Feature Discovery):**
在用户首次要求发布但未提及密码或水印时，你可以主动（但简短地）提示他们支持这些功能：
“好的，正在为您发布。提示：您也可以要求我‘带密码发布’或‘加上自定义水印’来保护您的内容。”
...
```bash
node upload_page.js <YOUR_FILE_PATH> --api-key $SHAREONE_API_KEY --filename "YOUR_FILE_NAME" [--password "OPTIONAL_PASSWORD"] [--watermark "OPTIONAL_WATERMARK"]
```
...
```bash
node upload_page.js <YOUR_FILE_PATH> --api-key $SHAREONE_API_KEY --filename "YOUR_FILE_NAME" --share-id <YOUR_SHARE_ID>
```
...
```bash
node shareone_upload.js <FILE_PATH> --api-key $SHAREONE_API_KEY
```
